...
const page = async ({ params }) => {
  ...
  const teams = Array.from(
    new Set(matchStats?.all_matches?.players.map((player) => player?.team_name))
  );
  ...
  if (teams?.length >= 2) {
    teamPlayersA = matchStats?.all_matches?.players?.filter((player) => {
      return player?.team_name === teams[0];
    });

    teamPlayersB = matchStats?.all_matches?.players?.filter((player) => {
    ...
    <>
      <div style={{ marginBottom: "70px" }} className={styles.page}>
        <div className={styles.main}>
          ...
          {matchStats?.length > 0 || matchStats?.all_matches ? (
            <>
              <CheatNav active={2} currentPath={currentPath} />
              <CheatDataVisual
                ...
                data={matchStats?.all_matches?.players.slice(0, 5)}
              />
              ...
              <CheatDataVisual
                ...
                teamNameA={matchDetails?.response?.teama?.short_name}
                teamNameB={matchDetails?.response?.teamb?.short_name}
                data={dreamTeamStats?.dream_team_stats?.overall?.slice(0, 5)}
                type="playerTime"
              />
              ...
              <CheatDataVisual
                ...
                teamNameA={matchDetails?.response?.teama?.short_name}
                teamNameB={matchDetails?.response?.teamb?.short_name}
                data={matchStats?.all_matches?.players.slice(-3)}
              />
            </>
          ) : (
            <>
